...
// Removed LicenseOfferModal import as we are using page navigation now
...
export function LicensingExplorer() {
    ...
    // Data Hooks
    const { tokens: myTokens, collections: myCollections, loading: myAssetsLoading, stats } = usePortfolio()
        ...
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center rounded-3xl glass border border-white/10 bg-black/20">
                ...
                <p className="text-muted-foreground max-w-md mb-8">
                    To manage your IP licenses or make offers on assets, you need to connect your Starknet wallet.
                </p>
            </div>
        )
    }
    ...
    return (
        <div className="space-y-8">
            ...
            <motion.div
                ...
                className="flex flex-col md:flex-row items-center justify-between gap-4 p-1.5 rounded-2xl glass border border-white/20 dark:border-white/10"
            >
                <Tabs
                    value={activeTab}
                    onValueChange={(val) => {
                ...
                <div className="relative w-full md:w-[300px] px-1">
                    ...
                    <Input
                        placeholder={activeTab === "my-assets" ? "Search your assets..." : "Search collection..."}
                        value={searchQuery}
            ...
            {activeTab === "my-assets" && isConnected && (
                <motion.div
                    initial={{ opacity: 0, y: 10 }}
                    animate={{ opacity: 1, y: 0 }}
                ...
                >
                    <Card className="bg-white/5 border-white/10 glass-card">
                        <CardContent className="p-4 flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-muted-foreground">Registered Assets</p>
                                <h4 className="text-2xl font-bold mt-1 text-foreground">{stats?.totalNFTs || 0}</h4>
                            </div>
                    ...
                    <Card className="bg-white/5 border-white/10 glass-card">
                        <CardContent className="p-4 flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-muted-foreground">Active IP Portfolios</p>
                                <h4 className="text-2xl font-bold mt-1 text-foreground">{myCollections.length || 0}</h4>
                            </div>
                    ...
                    <Card className="bg-white/5 border-white/10 glass-card">
                        <CardContent className="p-4 flex items-center justify-between">
                            <div>
                                ...
                                <h4 className="text-2xl font-bold mt-1 text-foreground">{stats?.recentActivity?.length || 0}</h4>
                            </div>
                                            ...
                                            >
                                                <LicensableAssetCard
                                                    ...
                                                    onLicense={() => {
                                                        ...
                                                        if (nftAddr) openLicensePage(asset, nftAddr);
                                                    }}
                                                    isOwner={true}
                                                />
                                            </motion.div>
...
function LicensableAssetCard({ asset, onLicense, isOwner }: { asset: any, nftAddress: string, onLicense: () => void, isOwner: boolean }) {
    return (
            ...
            <CardContent className="p-4 space-y-4 relative flex-grow flex flex-col justify-between">
                ...
                <div className="space-y-3 pt-2 border-t border-white/5">
                    ...
                    >
                        ...
                        {isOwner ? "Define License Terms" : "Request License"}
                    </Button>
                </div>
            </CardContent>
        </Card>
